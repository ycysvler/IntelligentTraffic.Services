...
class IVehicleStructure:
    ...
    def floatArrayAdapter(self, array):
        return array
        #results = []
        #for item in array:
        #    floatitem = round(float(item),4)
        #    results.append(floatitem)
        #return results
    ...
    def __writeVehicleResult(self,resultD3,result):
        ...
        for label in resultD3.keys():
            if label==1:
                ...
                temp = self.floatArrayAdapter(result['skyRoof'])               
                result['skyRoof'] = temp
            elif label==3:
    ...
    def __writeFrontGlassResult(self,resultD10,result):
        ...
        for label in resultD10.keys():
            if label in [1,2,9,10]:
                ...
                temp = self.floatArrayAdapter(result['sideSeatZone'])               
                result['sideSeatZone'] = temp
                ...
                #result['frontWindowSunShield']  = self.floatArrayAdapter(result['frontWindowSunShield'])    
            elif label==6:
                result['withFrontWindowObjects'] = True
                result['frontWindowObjectsZone'] = resultD10[label]
                #result['frontWindowObjectsZone']  = self.floatArrayAdapter(result['frontWindowObjectsZone'])
            elif label==7:
                result['withFrontWindowAccessories'] = True
                result['frontWindowAccessoriesZone'] = resultD10[label]
                #result['frontWindowAccessoriesZone']  = self.floatArrayAdapter(result['frontWindowAccessoriesZone'])
            elif label==8:
                result['withFrontWindowLabelInspection'] = True
                result['frontWindowLabelInspectionZone'] = resultD10[label]
                #result['frontWindowLabelInspectionZone']  = self.floatArrayAdapter(result['frontWindowLabelInspectionZone'])
        ...
        if score!=0:
            ...
            temp = self.floatArrayAdapter(result['driveSeatZone'])               
            result['driveSeatZone'] = temp
